    ...
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        ...
        return Ok(new
        {
            ...
            features = new
            {
                ...
                aiFillFromPdf = new { enabled = true, note = "Work-item AI fill from PDF text or, when the file is scanned/image-only, from rendered page images (vision). Fills Title, Type, Property IDs, counts, Worked date plus Easy / Medium / Hard difficulty from the same pass. Field fill is amber until Approve or edit; Save is still required for fields. Difficulty persists on a successful fill. Staff override (Editor+) sticks until cleared or a confirmed re-score. Never writes Status, Assignee, or flags. Fail closed unless AzureOpenAI__Endpoint and AzureOpenAI__ApiKey are set (Key Vault for the key). Deployment gpt-4.1-mini on oai-bis-deed-ai." },
