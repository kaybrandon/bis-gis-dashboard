    ...
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        ...
        return Ok(new
        {
            ...
            features = new
            {
                ...
                myQueue = new { enabled = true, note = "CR05 — Staff Manage Documents defaults to assigned to the signed-in user when that queue has work, Pending first then other statuses, oldest upload/created ascending. Global Admin or a 0 my-queue default to Assignee = All / All items so counts match that scope. Saved My queue presets (Assigned to me, Unassigned, Priority, Due this week, Clear) may override after the user sets them. Queues use work-item Assigned to (CR09), not org Assigned technician. Viewer/Uploader have no Assignee dropdown (QC08)." },
                ...
                wishList = new { enabled = true, note = "WL01–WL13 verify/close; pack complete after WL07 merge. WL01 staff all orgs (QC03). WL02 client upload into assigned org (QC04). WL03 separate Upload Documents multi-file (QC04/QC05). WL04 org auto on client upload; staff set type/priority. WL05 staff select org; batch stays that org. WL06 left-nav + Manage Documents + Upload open /upload-documents. WL07 status vocabulary Active · Pending · Complete · On-Hold · Cancelled · Needs Review (CR11); Needs Review ≠ Reviewed Yes/No. WL08 Deed, Plat, Survey, Subdivision, Other. WL09 Assigned to auto org Assigned technician (QC01/CR08/CR09). WL10 Reviewed next to Priority. WL11 Review Yes/No on Manage Documents. WL12 TIFF first-page + image preview (CR04). WL13 Manage Documents defaults to signed-in Assigned to (CR05/QC08)." },
                ...
                aiFillFromPdf = new { enabled = true, note = "Work-item AI fill from PDF, JPG/JPEG, PNG, TIFF/TIF, DOCX, and XLSX (first sheet only). Text Office uses extracted content; scanned images and image-only PDFs use rendered page images (vision). A successful supported-format upload kicks the same AI-fill + Easy / Medium / Hard pass automatically; AI fill remains for re-run / retry. DOC, XLS, macros, and other upload types are not analyzed. Unreadable, password-protected, or unsupported files fail with a clear scan error (never silent analyzed). Property IDs are manual-only — AI never fills, suggests, or overwrites them (auto-scan, AI fill, reanalysis, or Approve AI fields). Field fill is amber until Approve or edit; Save is still required for fields. Difficulty persists on a successful fill. Staff override (Editor+) sticks until cleared or a confirmed re-score. Never writes Status, Assignee, flags, or Property IDs. Upload still succeeds if Azure OpenAI is unconfigured or the scan fails (AI scan pending / failed). Fail closed for manual fill unless AzureOpenAI__Endpoint and AzureOpenAI__ApiKey are set (Key Vault for the key). Deployment gpt-4.1-mini on oai-bis-deed-ai." },
                ...
                connections = new { enabled = true, note = "Admin Connections: Source / Destination / Direction. Azure paths display and persist as workfiles/orgs/… (container + folder) in resource group rg-bis-gis-dashboard. Bare /orgs/… is the same Azure folder. Check folders shows Pass or Fail for the agent’s PC and Windows user. Sync errors show on the list and Edit." },
