    ...
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        ...
        return Ok(new
        {
            ...
            features = new
            {
                ...
                presence = new { enabled = true, note = "Signed-in staff (Global Administrator, Administrator, Editor) see who is online, the page or work item they are on, and a Clocked in badge when the floating clock is running. The list is a collapsible strip immediately under the top menu bar (not inside the greeting / org-chrome header row). Need help? raises a yellow hand on that strip. Light messages are online-only. Global Administrators also see it on Status. Viewers do not see other organizations’ presence. Offline after about three minutes without a heartbeat." },
