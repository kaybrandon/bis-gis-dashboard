    ...
    public async Task<IActionResult> Get(CancellationToken cancellationToken)
    {
        ...
        return Ok(new
        {
            ...
            features = new
            {
                ...
                myQueue = new { enabled = true, note = "CR05 — Staff Manage Documents defaults to assigned to the signed-in user, Pending first then other statuses, oldest upload/created ascending. Saved My queue presets (Assigned to me, Priority, Due this week, Clear) may override after the user sets them. Viewer/Uploader have no Assignee dropdown (QC08)." },
                ...
                dashboard = new { enabled = true, note = "KPI cards, scoped filters, date range, volume / hours charts, PDF download, and email of the current view. CR03 — Active tile count is the full matching Active total (not one page) for the current organization and assignee scope. CR05 — staff default to the signed-in assignee; Active opens that same queue." },
                ...
                dashboardAssignee = new { enabled = true, note = "QC08 — Dashboard Assignee is hidden for Viewer and Uploader. Dashboard choices, charts, counts, and document results stay assigned-org only. Filter params and direct API cannot open another org. Editors and Administrators keep authorized cross-org filtering (QC03). CR05 — staff default to the signed-in assignee and may switch to all or another person; Viewer/Uploader never get a cross-org Assignee control." },
                ...
                presence = new { enabled = true, note = "Signed-in staff (Global Administrator, Administrator, Editor) see who is online, the page or work item they are on, and a Clocked in badge when the floating clock is running. The list is a collapsible panel on the left sider immediately under Status (not a header / menu-bar strip). Need help? raises a yellow hand on that panel. Light messages are online-only. A header envelope lists waiting Need-help threads (including offline senders) with an unread badge; opening a thread uses the same light sheet and marks it read. Global Administrators also see it on Status. Viewers do not see other organizations’ presence. Offline after about three minutes without a heartbeat." },
