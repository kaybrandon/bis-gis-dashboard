...
**QC08 — Dashboard Assignee:** Viewer and Uploader do not see the Dashboard Assignee dropdown (`canSeeDashboardAssignee=false`). Dashboard organization choices, chart labels, KPI counts, and document results expose only assigned-org data. Passing another organization’s id on dashboard, PDF, or work-item APIs returns 404. Assignee lookup is empty for those roles. Hidden UI is not isolation — the API drops `assignedToUserId` for Viewer/Uploader. Editors and Administrators keep authorized cross-org filtering (QC03).
