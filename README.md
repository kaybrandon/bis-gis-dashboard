...
**Who’s online:** while signed in, the browser sends a heartbeat about every 45 seconds and on each route change (current page, work item when on `/documents/{id}`, and whether the floating clock is running). Staff see a people icon in the header and a collapsible strip immediately under the top menu bar on main signed-in routes (not Dashboard-only). The strip is compatible with a Need help raise-hand on the same chrome. Global Administrators also have a detailed list on **Status**. Global Administrators see everyone; Editors and organization Administrators see people who share an organization (plus themselves and Global Administrators). Client Viewers do not get the list. Offline after about three minutes without a heartbeat. There is no chat or screen share.
