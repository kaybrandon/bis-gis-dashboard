...
The Dashboard date range (default last 30 days) filters pending/completed/priority charts and **document counts by CAD and technician** (CR10). **Active** is the current-state count for the selected organization and assignee (CR03) — it matches the Manage Documents Active list total, including rows beyond the first page, and is not limited to the date window. Staff with assigned work default to the signed-in assignee (CR05); Global Admin and a 0 my-queue default Manage Documents to all assignees. They can switch to all or another person. **Download PDF** and **Send report** use the same organization / status / assignee / date filters. Send asks for confirmation and fails closed when email is not configured.
...
**CR05 — Signed-in queue:** Staff Manage Documents and Dashboard default to items assigned to the signed-in user when that personal queue has work. Global Admin, or staff whose my-queue count is 0, default Manage Documents to **Assignee = All / All items** so sidebar counts match that scope on load. Default order is Pending first, then other statuses, oldest upload/created ascending. Saved My queue presets may override after the user sets them. Authorized staff can switch to all, Unassigned, or another assignee; counts follow the selected scope. Viewer/Uploader have no Assignee dropdown and see only assigned-org data (QC08).
...
**Manage Documents** (Editor+): status and assignee can be changed inline. Staff with assigned work default to **assigned to me**, Pending first, oldest upload. Global Admin and a 0 my-queue default to **Assignee = All / All items**. **My queue** presets — Assigned to me, Unassigned, Priority, Due this week, Clear — persist in `localStorage` (`gis.myQueue`) and may override that default. Viewer/Uploader do not see the Assigned to filter.
