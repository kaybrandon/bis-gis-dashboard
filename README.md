...
**CR05 — Signed-in queue:** Staff Manage Documents and Dashboard default to items assigned to the signed-in user. Default order is Pending first, then other statuses, oldest upload/created ascending. Saved My queue presets may override after the user sets them. Authorized staff can switch to all or another assignee; counts follow the selected scope. Viewer/Uploader have no Assignee dropdown and see only assigned-org data (QC08).
...
**Manage Documents** (Editor+): status and assignee can be changed inline. Staff default to **assigned to me**, Pending first, oldest upload. **My queue** presets — Assigned to me, Priority, Due this week, Clear — persist in `localStorage` (`gis.myQueue`) and may override that default. Viewer/Uploader do not see the Assignee filter.
